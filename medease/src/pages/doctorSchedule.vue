  ...
  export default {
    data() {
      return {
        ...
        timeSlots: Array.from({ length: 16 }, (_, i) => `${8 + Math.floor(i / 2)}:${i % 2 === 0 ? '00' : '30'}`),
        appointments: {},
    ...
    methods: {
      openAppointment(time, day) {
        if (this.appointments[day] && this.appointments[day][time]) {
          this.selectedAppointment = { ...this.appointments[day][time], time, day };
        }
      },
      addAppointment(time, day) {
        const patientName = prompt("Enter Patient Name (First and Last):");
        if (patientName) {
          if (!this.appointments[day]) this.appointments[day] = {};
          const patientDetails = this.patients.find(p => `${p.firstName} ${p.lastName}`.toLowerCase() === patientName.toLowerCase());
          this.appointments[day][time] = {
            patientName,
